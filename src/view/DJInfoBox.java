...
public class DJInfoBox {
	...
	public DJInfoBox(Owner o){
		...
		for(int i = 0; i < o.getpBag().getSize(); i++){
			for(int j = 0; j < o.getpBag().getManagers()[i].getEmployees().getDJs().length; j++){
				djBox.getItems().add(o.getpBag().getManagers()[i].getEmployees().getDJs()[j].getfName() + 
						" " + o.getpBag().getManagers()[i].getEmployees().getDJs()[j].getlName() + " " + 
						o.getpBag().getManagers()[i].getEmployees().getDJs()[j].getId());
			}
		}
		...
		djBox.setOnAction(e->{
			String arr[] = djBox.getValue().split(" ");
			for(int i = 0; i < o.getpBag().getSize(); i++){
				for(int j = 0; j < o.getpBag().getManagers()[i].getEmployees().getSize(); j++){
					if(o.getpBag().getManagers()[i].getEmployees().findPerson(arr[2]) != null){
						dj = (DJ) o.getpBag().getManagers()[i].getEmployees().findPerson(arr[2]);
		...
		rootPane.getChildren().addAll(infoTxt, djBox, subrootPane);
	...
	private void addPane(DJ dj){
		subrootPane.getChildren().clear();
		fNameFld.setText( dj.getfName());
		lNameFld.setText(dj.getlName());
		phoneFld.setText(dj.getPhone());
		stNameFld.setText(dj.getAddress().getStName());
		stNumberFld.setText(dj.getAddress().getStNumber());
		cityFld.setText(dj.getAddress().getCity());
		stateFld.setText(dj.getAddress().getState());
		zipFld.setText(dj.getAddress().getZip());
		salaryFld.setText(dj.getSalary());
